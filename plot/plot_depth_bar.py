# -*- coding: utf-8 -*-
"""
plot_depth_bar: Normalized depth bars (Fig.16-like)
- For each circuit: draw Ideal (=1.0) + methods (trivial/sabre/vf2/dense)
- Methods show mean(depth_constrained / depth_ideal) with std error bars (successful runs only)
- Failure detection still uses cx_added == -1 (compile failed)
- "Perfect depth" ✓ counts where depth_constrained == depth_ideal among successful runs
- √ y-scale, top-centered legend, academic palette

Inputs:
  ../results/b23_Tokyo20.csv   (adjust IN_CSV if needed)
Outputs:
  ../results/depth_norm_bar.png
  ../results/depth_norm_stats.csv
"""
...
def plot_depth_bar(in_path: str, out_path: str):
    # ------------ Config ------------
    IN_CSV = Path(in_path)
    OUT_DIR = Path(out_path)

    FIG_PATH = OUT_DIR / "depth_norm_bar.png"
    # STATS_PATH = OUT_DIR / "depth_norm_stats.csv"
...
if __name__ == "__main__":
    plot_depth_bar("../results/b23_Tokyo20_mapping.csv", "../results")