# nohup python3 2_table_second_scale.py > 2_table_second_scale.output 2>&1 &
import os
import time
import csv
import signal
...
problem_scale = 2 # The problem scale, 1 is the minimal scale like F1,K1,G1 in Table 1 of paper
#2 means F2 K2 ... In CPU version, this benchmarks with higher scale is much slower when solving with baselines.

flp_problems_pkg, flp_configs_pkg = generate_flp(num_cases, [(3, 3), (3, 4)][:problem_scale], 1, 20)
gcp_problems_pkg, gcp_configs_pkg = generate_gcp(num_cases, [(4, 2), (4, 3)][:problem_scale])
kpp_problems_pkg, kpp_configs_pkg = generate_kpp(num_cases, [(8, 3, 7), (9, 3, 8)][:problem_scale], 1, 20)

configs_pkg = flp_configs_pkg + gcp_configs_pkg + kpp_configs_pkg
with open(f"2_table_second_scale.config", "w") as file:
    for pkid, configs in enumerate(configs_pkg):
        for problem in configs:
            file.write(f'{pkid}: {problem}\n')

new_path = '2_table_depth_second_scale'
...
file_path = '2_table_depth_second_scale.csv'
...
new_path = '2_table_evaluate_second_scale'
...
file_path1 = '2_table_depth_second_scale.csv'
...
file_path2 = '2_table_evaluate_second_scale.csv'
...
merged_df.index = ['F1', 'F2', 'G1', 'G2', 'K1', 'K2']
